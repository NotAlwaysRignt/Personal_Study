...
static inline rpchook_t * alloc_by_fd( int fd )
{
	if( fd > -1 && fd < (int)sizeof(g_rpchook_socket_fd) / (int)sizeof(g_rpchook_socket_fd[0]) )
	{
		rpchook_t *lp = (rpchook_t*)calloc( 1,sizeof(rpchook_t) ); //calloc会自动初始化0
		lp->read_timeout.tv_sec = 1; //默认设为1秒
...
//创建非阻塞socket,有三种思路:在调用socket函数时传参,使用fcntl,使用ioctl
//这里使用的是fcntl的方式(fcntl也被hook了)设置fd的属性：flag |= O_NONBLOCK;
int socket(int domain, int type, int protocol)
{
  ...
  //socket创建成功,将fd加入 g_rpchook_socket_fd 变量
	rpchook_t *lp = alloc_by_fd( fd );
	lp->domain = domain;
	//将socket修改为非阻塞的关键在下面fcntl，注意这里的fcntl是被HOOK了的
	fcntl( fd, F_SETFL, g_sys_fcntl_func(fd, F_GETFL,0 ) ); // 被hook后的fcntl，当使用F_SETFL时,一律会使用 flag |= O_NONBLOCK;

	return fd;
}
//调用accept,但加了一步,将fd 注册到 g_rpchook_socket_fd 中
...
ssize_t read( int fd, void *buf, size_t nbyte )
{
	...
	int timeout = ( lp->read_timeout.tv_sec * 1000 ) //调用被hook的socket时,初始化read_timeout.tv_sec被赋值为固定值1
	...
	int pollret = poll( &pf,1,timeout ); //poll被hook了,最终调用 co_poll,而co_poll 被调用时相当于向epoll添加了监听事件,然后就会切换协程
  // 协程在timeout时间内如果有读事件到来,则会切换回来,并执行下面的read,或者超过timeout时间后,也会执行下面的read
  // 这里调用poll 而非直接read,在当前还没有读事件时,则可以先切出协程等待一段时间,但又不会阻塞,增加read的成功率.如果当前可读,那也会被poll监听

	ssize_t readret = g_sys_read_func( fd,(char*)buf ,nbyte ); //此时系统的read已经在调用被hook的socket函数中将read设置为非阻塞的read

	if( readret < 0 ) // 连接出错,或者当前没有可读的数据(客户端仍然保持连接,但没有数据可读,对于非阻塞read也会返回负数)
	{
...
ssize_t write( int fd, const void *buf, size_t nbyte )
{
	...
	if( !lp || ( O_NONBLOCK & lp->user_flag ) ) //已经向g_rpchook_socket_fd中注册了该文件描述符
	{
		ssize_t ret = g_sys_write_func( fd,buf,nbyte );//调用非阻塞的write
		return ret; // 已经向g_rpchook_socket_fd注册过了，就不用注册了，这里返回即可
	}
	size_t wrotelen = 0; // 此处往下的逻辑是还为向g_rpchook_socket_fd注册时（第一次调用被hook的write）时要执行的
	int timeout = ( lp->write_timeout.tv_sec * 1000 ) 
				+ ( lp->write_timeout.tv_usec / 1000 );

	ssize_t writeret = g_sys_write_func( fd,(const char*)buf + wrotelen,nbyte - wrotelen ); //此时系统的write 已经在调用被hook的socket函数中将write设置为非阻塞的write
	...
	while( wrotelen < nbyte )
	{
		...
		pf.fd = fd;
		pf.events = ( POLLOUT | POLLERR | POLLHUP );
		poll( &pf,1,timeout ); //调用系统的 write 之前先调用 poll ,注意这里的poll被hook了,最终调用的是co_poll,它将写事件添加交给epoll监听,然后切换出去,如果有网络事件到来,则可以切换回此处往下执行系统的 write,如果一直没有网络写事件,则在timeout期间内可以处理别的协程任务,在 timeout 时间后也会切换回来执行系统的write,如果 write 失败再做错误处理
...
int poll(struct pollfd fds[], nfds_t nfds, int timeout)
{
	...
	if (nfds > 1) {
		fds_merge = (pollfd *)malloc(sizeof(pollfd) * nfds); //fds_merge的空间与fds一致
		for (size_t i = 0; i < nfds; i++) {
			if ((it = m.find(fds[i].fd)) == m.end()) { //如果m 里面没有fd这个key
  ...
  //fds_merge相当于把fds给"合并"",fd相同的只在fds_merge中存在一个元素,event以或的形式运算后存入fds_merge
  //最终 m 中存放的是以 fd 为key(fds中的fd会有重复,而m中不会有重复),对应的value是fds_merge中该fd对应的索引
	int ret = 0;
	if (nfds_merge == nfds || nfds == 1) { //调用被hook的write时,nfds固定传入1,timeout为1000(ms)
